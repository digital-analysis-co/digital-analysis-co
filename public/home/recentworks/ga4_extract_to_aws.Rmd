...
Once Github Actions are set up and authorization is done (steps further down), you should also see a screen like this when testing the workflow. If you test beforehand setting things up you will get a red failed message instead. The workflow runs with the default date in DATE_PART if ebing run while testing.
...
The final code block iterates over each date. 
First some environment vars are set and then gcloud is authorized using the service account json. Next data are extracted using gsutil to Google Storage using `bq extract`. The data are Rsynced from Google Storage to AWS S3.
